...
def enrich(filename):
    ...
    start_time, refined, nlp, stop_words = setup_functions.initilize_with_models('enrich_'+str(args.capitalization)+'_'+os.path.splitext(os.path.basename(filename))[0])
    logging.info(filename)
    output_file = args.output_dir + 'enriched_'+str(args.capitalization)+'_'+os.path.splitext(os.path.basename(filename))[0]+'.csv'
    logging.info(output_file)
    dataset = manage_datasets_functions.read_csv_dataset(filename=filename, separator=';')
    ...
    dataset = enrich_functions.enrich(dataset=dataset, option=args.capitalization, refined=refined, nlp=nlp, stop_words=stop_words)
...
def main():
    ...
    if args.enrich_mode == 'folder':
        ...
        pool.map(enrich, [os.path.join(args.tmp_dir, os.path.basename(file)) for file in files])
        pool.close()
        pool.join()
    else:
